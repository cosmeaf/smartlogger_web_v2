...
import { FaEdit, FaArrowLeft, FaSave, FaWrench, FaCog, FaClock, FaSpinner } from 'react-icons/fa';
...
const MaintenanceEdit = () => {
  ...
  const validateForm = () => {
    ...
    if (!maintenance.name.trim()) {
      newErrors.name = 'Nome da peça é obrigatório';
    } else if (maintenance.name.length < 2) {
      newErrors.name = 'Nome deve ter pelo menos 2 caracteres';
    }
    
    if (!maintenance.initial_hour_maintenance || maintenance.initial_hour_maintenance < 0) {
      newErrors.initial_hour_maintenance = 'Hora inicial deve ser um número positivo';
    }
    
    if (!maintenance.alarm_hours || maintenance.alarm_hours <= 0) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSaving(true);
      const formattedData = {
        name: maintenance.name.trim(),
        initial_hour_maintenance: parseFloat(maintenance.initial_hour_maintenance) || 0,
        alarm_hours: parseFloat(maintenance.alarm_hours) || 0,
        obs: maintenance.obs.trim(),
      };
      ...
      toast.error(`Falha ao atualizar manutenção: ${error.response?.data?.detail || 'Erro desconhecido'}`);
    } finally {
      setSaving(false);
    }
  };
    ...
    return (
      <div className="p-6 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className={`text-lg ${isDarkMode ? 'text-gray-300' : 'text-gray-600'}`}>Carregando manutenção...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'} mt-1`}>
              Equipamento: {maintenance.equipment.name} • Device ID: {maintenance.equipment.device}
            </p>
          </div>
        </div>
      </div>
        ...
        <form onSubmit={handleSubmit} className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} rounded-xl shadow-lg border overflow-hidden`}>
          ...
          <div className="p-6">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                <label htmlFor="equipment_name" className={`flex items-center gap-2 text-sm font-medium ${isDarkMode ? 'text-gray-300' : 'text-gray-700'}`}>
                ...
                <div className="relative">
                  <input
                    ...
                    name="equipment_name"
                    id="equipment_name"
                    value={maintenance.equipment.name}
